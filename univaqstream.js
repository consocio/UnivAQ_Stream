...
const argv = yargs.options({
    ...
    u: { alias:'username', type: 'string', demandOption: true, describe: 'Codice utente UnivAQ' },
...
async function downloadVideo(videoUrls, username, password, outputDirectory) {
   ...
   //keytar.deletePassword('UnivAQ', username);
   if(password === null) { // password not passed as argument
        var password = {};
        if(argv.noKeyring === false) {
          try {
              await keytar.getPassword("UnivAQ", username).then(function(result) { password = result; });
              if (password === null) { // no previous password saved
                  password = await promptQuestion("Password not saved. Please enter your password, UnivAQ will not ask for it next time: ");
                  await keytar.setPassword("UnivAQ", username, password);
              } else {
                  console.log("Reusing password saved in system's keychain!")
              }
          }
          catch(e) {
              console.log("X11 is not installed on this system. UnivAQ can't use keytar to save the password.")
